            ...
            WindowKind::Window(.., ref events_loop) => {
                ...
                if !close {
                    events_loop.borrow_mut().poll_events(|event| {
                        self.polled_winit_events.borrow_mut().push_back(event);
                    });
                    while let Some(event) = self.polled_winit_events.borrow_mut().pop_front() {
        ...
        if poll {
            ...
            match self.kind {
                WindowKind::Window(.., ref events_loop) => {
                    ...
                    while let Some(event) = polled_events.iter().next() {
                        close_event = self.handle_window_event(event) || close_event;
                    }
                }
                WindowKind::Headless(..) => {}
