...
pub enum ServoBrowser {
    Invalid,
    OnScreen(Servo<windowing_app::window::Window>, BrowserId),
...
impl ServoCefBrowser {
    pub fn new(window_info: &cef_window_info_t, client: CefClient, target_url: ServoUrl) -> ServoCefBrowser {
        ...
        let (window, servo_browser) = if window_info.windowless_rendering_enabled == 0 {
            let parent_window = windowing_app::WindowID::new(window_info.parent_window as *mut _);
            let window = windowing_app::create_window(Some(parent_window));
            ...
            window_handle = window.platform_window().window as cef_window_handle_t;
            (Some(window), ServoBrowser::OnScreen(servo_browser, browser_id))
        } else {
