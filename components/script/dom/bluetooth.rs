...
use dom::bindings::codegen::Bindings::BluetoothBinding::{self, BluetoothLEScanFilterInit};
...
const KEY_CONVERSION_ERROR: &'static str = "This `manufacturerData` key can't be converted to number: ";
...
const MASK_LENGTH_ERROR: &'static str = "`mask` if present, must hace the same length as `dataPrefix`.";
    ...
    let manufacturer_data = match filter.manufacturerData {
        Some(ref manufacturer_data_map) => {
            ...
            for (key, bdfi) in manufacturer_data_map.deref() {
                ...
                map.insert(manufacturer_id, canonicalized_data_filter);
            }
            Some(map)
        },
        None => None,
        ...
        Some(ref service_data_map) => {
            ...
            for (key, bdfi) in service_data_map.deref() {
                let service_name = match u32::from_str(key.as_ref()) {
                    Ok(number) => {
                        try!(BluetoothUUID::service(StringOrUnsignedLong::UnsignedLong(number))).to_string()
                    },
                    _ => try!(BluetoothUUID::service(StringOrUnsignedLong::String(key.clone()))).to_string()
                };
                if uuid_is_blacklisted(service_name.as_ref(), Blacklist::All) {
                    return Err(Security);
                }
                let canonicalized_data_filter = {
                    let data_prefix = match bdfi.dataPrefix {
                ...
                map.insert(service_name, canonicalized_data_filter);
            }
            Some(map)
        },
        None => None,
