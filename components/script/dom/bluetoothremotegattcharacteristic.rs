...
impl BluetoothRemoteGATTCharacteristic {
    ...
    pub fn is_represented_characteristic_null(&self) -> bool {
        let (sender, receiver) = ipc::channel().unwrap();
        self.get_bluetooth_thread().send(
            BluetoothRequest::IsRepresentedNull(self.get_instance_id(), GATTLevel::Characteristic, sender)).unwrap();
        let result = receiver.recv().unwrap();
        result.unwrap_or(true)
    }

    pub fn set_represented_characteristic_to_null(&self) {
        self.get_bluetooth_thread().send(
            BluetoothRequest::SetRepresentedToNull(self.get_instance_id(), GATTLevel::Characteristic)).unwrap()
    }
}
